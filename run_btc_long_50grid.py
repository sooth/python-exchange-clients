#!/usr/bin/env python3
"""Run BTC LONG 50-Grid Bot with Position Verification"""
...
def main():
    ...
    print("""
╔═══════════════════════════════════════════════════════════╗
║          BTC LONG 50-Grid Bot - Tight Range Trading        ║
╚═══════════════════════════════════════════════════════════╝
""")
    ...
    print("Configuration loaded from btc_long_50grid_config.json")
    ...
    print(f"Direction: {grid_config.position_direction.value} 📈")
    ...
    print(f"- Tight range ({((grid_config.upper_price - grid_config.lower_price) / grid_config.lower_price * 100):.2f}%) means frequent trading")
    print(f"- Monitor closely for breakouts")
    ...
    print("\n" + "="*60)
    print("Ready to start the BTC LONG 50-grid bot")
    print(f"⚠️  This will manage ${grid_config.total_investment} with {grid_config.leverage}x leverage")
    print(f"⚠️  50 grid orders will be placed in a tight range!")
    print("="*60)
        ...
        while bot.state.value == "running":
            ...
            if current_time - last_update >= 5:
                ...
                print(f"\r📊 WebSocket: {'✅' if bot.websocket_connected else '❌'} | "
                      ...
                      f"Grid P&L: ${stats.get('grid_profit', 0):.2f} | "
                      f"Pos P&L: ${position.get('unrealized_pnl', 0):.2f} | "
                      f"Total: ${stats.get('total_profit', 0):.2f} | "
                      f"Trades: {stats.get('trades', 0)} | "
                      f"Orders: {orders.get('active_count', 0)} | "
                      f"Pos: {position.get('size', 0):.6f} @ ${position.get('entry_price', 0):,.2f}",
                      end="", flush=True)
    ...
    if bot_instance:
        ...
        print(f"Grid Profit: ${stats.get('grid_profit', 0):.2f}")
        print(f"Position P&L: ${position.get('unrealized_pnl', 0):.2f}")
        print(f"Total Profit: ${stats.get('total_profit', 0):.2f}")
        print(f"Win Rate: {stats.get('win_rate', 0):.1f}%")
        print(f"Final Position: {position.get('size', 0):.6f} BTC")
        
        if abs(position.get('size', 0)) > 0:
            print(f"\n⚠️  WARNING: You still have an open position of {position.get('size', 0):.6f} BTC")
