...
from exchange_interface import ExchangeInterface, PositionSide
...
class BitUnixAdapter(ExchangeInterface):
    ...
    def placeOrder(self, request: ExchangeOrderRequest, completion: Callable[[Tuple[str, Any]], None]):
        ...
        # Log order request
        self.logger.info(f"Placing order - Symbol: {request.symbol}, Side: {request.side}, Qty: {request.qty}, Type: {request.orderType}")
        self.logger.debug(f"Order details - Price: {request.price}, StopLoss: {request.stopLoss}, TakeProfit: {request.takeProfit}")
        ...
        def logged_completion(result):
            status, data = result
            if status == "success":
                ExchangeLogger.log_order_placement(self.logger, request.__dict__, True, data.rawResponse if hasattr(data, 'rawResponse') else data)
            else:
    ...
    def cancelOrder(self, orderID: str = None, clOrderID: str = None, symbol: str = None, 
        ...
        self.logger.info(f"Cancelling order - OrderID: {orderID}, ClientID: {clOrderID}, Symbol: {symbol}")
        
        # Wrap completion to add logging
        def logged_completion(result):
            status, data = result
            ExchangeLogger.log_order_cancellation(self.logger, orderID or clOrderID, symbol, status == "success", data)
            if completion:
                completion(result)
    ...
    def validateAndAdjustPrice(self, symbol: str, price: float, side: str) -> float:
        ...
        if side.upper() in ['BUY', 'LONG']:
            ...
            if current_price < 10000 and 'BTC' in symbol:
                print(f"[BitUnix] Detected low mock price ${current_price:.2f}, using realistic BTC price")
            ...
            if price < min_buy_price:
                ...
                print(f"[BitUnix] Adjusting buy price from ${price:.2f} to ${adjusted_price:.2f}")
