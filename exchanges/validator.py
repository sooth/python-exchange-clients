...
class ExchangeValidator:
    ...
    def validate_all(self, include_trading: bool = False,
                     include_websocket: bool = True) -> Dict[str, Any]:
        """
    ...
    def _validate_basic_interface(self):
        ...
        result = self._test_method(
            ...
            lambda: self.exchange.translate_side_to_exchange(
                PositionSide.LONG),
            lambda side: side in [
                "BUY",
                "LONG",
    ...
    def _test_method(self, name: str, method: Callable,
    ...
    def _test_async_method(
        ...
        try:
            ...
            # Wait for completion
            timeout = 10
            while not result_data["completed"] and time.time(
            ) - start_time < timeout:
                time.sleep(0.1)
            ...
            if result_data["status"] == "success":
                ...
                return ValidationResult(
                    ...
                    message=f"❌ Failed: {
                        result_data['data']}",
                    duration=duration,
                    error=result_data["data"] if isinstance(
                        result_data["data"],
                        Exception) else None)
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        for category, tests in categories.items():
            category_results = [
                r for r in self.results if r.test_name in tests]
            if category_results:
                print(f"\n{category}:")
                for result in category_results:
                    ...
                    if result.error:
                        print(
                            f"     Error: {type(result.error).__name__}: {str(result.error)}")
