...
class BaseWebSocketManager(WebSocketProtocol):
    ...
    def connect(self, url: str, on_open: Callable = None, on_close: Callable = None) -> bool:
        ...
        """
        with self._lock:
            if self._state not in [WebSocketState.DISCONNECTED, WebSocketState.ERROR]:
                logger.warning(f"{self.exchange_name}: Already connected or connecting")
                return False
            ...
            self._ws = websocket.WebSocketApp(
                url,
                on_open=lambda ws: self._on_open(ws, on_open),
                on_close=lambda ws, code, msg: self._on_close(ws, code, msg, on_close),
    ...
    def disconnect(self):
        ...
        if hasattr(self, '_message_thread') and self._message_thread and self._message_thread.is_alive():
        ...
        for thread in threads_to_join:
            ...
            if thread.is_alive():
                logger.warning(f"{self.exchange_name}: Thread {thread.name} did not terminate gracefully")
    ...
    def subscribe(self, channels: List[Dict[str, Any]]) -> bool:
        ...
        with self._lock:
            ...
            if current_count + len(channels) > max_subscriptions:
                logger.warning(f"{self.exchange_name}: Subscription limit reached ({current_count} + {len(channels)} > {max_subscriptions})")
                # Only add what we can
                available_slots = max(0, max_subscriptions - current_count)
                if available_slots > 0:
                    channels = channels[:available_slots]
                    logger.info(f"{self.exchange_name}: Limited to {available_slots} new subscriptions")
                else:
                    logger.error(f"{self.exchange_name}: Cannot add more subscriptions, limit reached")
                    return False
            ...
            if not self.is_connected():
                ...
                for channel in channels:
                    # Convert to string for comparison to avoid dict comparison issues
                    channel_str = json.dumps(channel, sort_keys=True)
                    existing_strs = [json.dumps(c, sort_keys=True) for c in self._pending_subscriptions]
                    if channel_str not in existing_strs:
                        self._pending_subscriptions.append(channel)
                        new_channels.append(channel)
                if new_channels:
                    logger.info(f"{self.exchange_name}: Queued {len(new_channels)} new subscriptions")
                return True
    ...
    def is_connected(self) -> bool:
        """Check if connected"""
        return self._state in [WebSocketState.CONNECTED, WebSocketState.AUTHENTICATED]
    ...
    def _on_open(self, ws, custom_callback: Optional[Callable] = None):
        ...
        # Send pending subscriptions in batches (BitUnix has a limit of 300 channels)
        with self._lock:
            if self._pending_subscriptions:
                ...
                for i in range(0, total_subs, batch_size):
                    ...
                    logger.info(f"{self.exchange_name}: Sending batch {i//batch_size + 1} with {len(batch)} subscriptions")
    ...
    def _on_close(self, ws, code: int, message: str, custom_callback: Optional[Callable] = None):
        """Handle WebSocket connection close"""
        logger.info(f"{self.exchange_name}: WebSocket closed - code: {code}, message: {message}")
        ...
        if self._should_reconnect and self.reconnect_config.enabled:
            if self.reconnect_config.max_attempts == -1 or self._reconnect_attempts < self.reconnect_config.max_attempts:
    ...
    def _run_websocket(self):
        ...
        while self._running:
            try:
                if self._ws:
                    logger.info(f"{self.exchange_name}: Starting WebSocket connection")
                    self._ws.run_forever(
                        ping_interval=self._heartbeat_interval,
                        ping_timeout=10
                    )
            except Exception as e:
                logger.error(f"{self.exchange_name}: WebSocket thread error: {e}")
                self._handle_error(e)
    ...
    def _schedule_reconnect(self):
        ...
        logger.info(f"{self.exchange_name}: Scheduling reconnection attempt {self._reconnect_attempts} in {self._reconnect_delay:.1f}s")
        ...
        if self._state != state:
            ...
            if self.on_state_change:
                try:
                    ...
                    logger.error(f"{self.exchange_name}: Error in state change callback: {e}")
    ...
    def _handle_error(self, error: Exception):
        ...
        if self.on_error:
            try:
                ...
                logger.error(f"{self.exchange_name}: Error in error callback: {e}")
    
    def _create_unified_message(self, channel: str, symbol: Optional[str], data: Any) -> WebSocketMessage:
