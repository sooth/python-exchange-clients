...
# Module logger
_logger = logging.getLogger("exchange.precision")
...
class SymbolPrecisionManager:
    ...
    def __init__(self, exchange_name: str):
        ...
        self.cache_path = os.path.join(
            os.path.dirname(__file__), self.cache_file)
    ...
    def has_symbol(self, symbol: str) -> bool:
        """Check if a symbol exists in the cache (alias for is_symbol_supported)"""
        return self.is_symbol_supported(symbol)
    ...
    def get_contract_size(self, symbol: str) -> float:
        ...
        if contract_size is None:
            raise ValueError(
                f"Contract size not found for {symbol}. "
                "Please ensure fetchTickers() has been called to populate symbol data.")
        ...
        try:
            if os.path.exists(self.cache_path):
                with open(self.cache_path, 'r') as f:
                    data = json.load(f)
                    # Support both formats: 'symbols' and 'precision_cache'
                    # (BitUnix format)
                    self.precision_cache = data.get(
                        'symbols', data.get('precision_cache', {}))
                    ...
                    _logger.debug(
                        f"Loaded precision cache with "
                        f"{len(self.precision_cache)} symbols from {self.cache_file}")
        except Exception:
            _logger.debug("Error loading precision cache")
    ...
    def _save_cache(self):
        ...
        try:
            ...
            _logger.debug(
                f"Saved precision cache with {len(self.precision_cache)} "
                f"symbols to {self.cache_file}")
        except Exception:
            _logger.debug("Error saving precision cache")
